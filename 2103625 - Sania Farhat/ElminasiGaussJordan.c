...
void gaussJordan(double arr[size][size], int ordo){
    ...
    // perulangan untuk semua matriks
    for(i = 0; i < ordo; i++){
        pembagi = arr[i][i];

        // perulangan matriks tiap baris
        for(k = i; k < ordo + 1; k++){
            arr[i][k] /= pembagi;
        }

        // perulangan matriks tiap kolom dan untuk mengecek secara vertikal agar array selain x[i][i] bernilai 0
        for(j = i + 1; j < ordo + 1; j++){
            v[i] = arr[j][i];

            // perulangan matriks kolom yang sama tiap baris
            for(k = i; k < ordo + 1; k++){
...
int main(){
    ...
    printf("SOLUSI PERSAMAAN SISTEM LINEAR METODE GAUSS JORDAN\n");
    ...
    // menampilkan nilai dari x1, x2, x3
    for(i = 0; i < ordo; i++){
